...
export default function PaymentForm() {
  ...
  const handleCheckout = async (price)=>{
    ...
    const config = { headers: { "Content-Type": "application/json" } };
     const res = await axios.post("http://localhost:8000/api/checkout",{amount:price},{withCredentials:true},config)
     const {data:{key}} = await axios.get("http://localhost:8000/api/getPay",{withCredentials:true},config)
